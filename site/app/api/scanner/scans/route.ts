...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { scanId, propertyName } = await request.json() as { scanId?: string; propertyName?: unknown };
    if (!scanId || !/^[0-9a-f-]{36}$/i.test(scanId)) throw new ApiError(400, "scan_required", "A scan ID is required.");
    const property = typeof propertyName === "string" && propertyName.trim() ? propertyName.trim().slice(0, 120) : null;
    const { error } = await auth.admin.from("scans").upsert({ id: scanId, owner_id: auth.userId, trace_id: traceId, property_name: property }, { onConflict: "id", ignoreDuplicates: true });
