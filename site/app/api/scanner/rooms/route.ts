...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { scanId, roomId, name, ordinal, targetCount = 0, captureMode } = await request.json();
    const mode = captureMode === "full3d" ? "full3d" : "quick";
    if (typeof name !== "string" || name.length > 120 || !Number.isInteger(targetCount) || targetCount < 0 || targetCount > 300) throw new ApiError(400, "invalid_room", "Room name or plan size is invalid.");
    ...
    const { error } = await auth.admin.from("rooms").upsert({ id: roomId, scan_id: scanId, owner_id: auth.userId, name, ordinal, target_count: targetCount, capture_mode: mode, trace_id: traceId }, { onConflict: "id", ignoreDuplicates: true });
