...
describe("POST /api/astra", () => {
  it("stays server-disabled when the key is absent", async () => {
    ...
    const response = await POST(new Request("http://localhost/api/astra", { method: "POST", body: "{}" }));
  ...
  it("sends bounded image input and returns structured review data", async () => {
    ...
    const review = { verdict: "usable", summary: "Usable capture.", guidance: "Retake the darkest corner.", issues: [] };
    ...
    const response = await POST(new Request("http://localhost/api/astra", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ roomName: "Living room", captured: 12, targetCount: 12, images: Array(7).fill("data:image/png;base64,AA==") }),
    }));
    ...
    expect(request.input[0].content.filter((item: { type: string }) => item.type === "input_image")).toHaveLength(4);
  });
});
